...
class TriviaState {
  ...
  getScoreData() {
    return {
      ...
      percentage: this.questionsAsked > 0 ? (this.score / this.questionsAsked) * 100 : 0,
    };
  }
}
...
After giving feedback, ask if they want another question.`,
      tools: {
        ...
        check_user_answer: llm.tool({
          ...
          execute: async (
            ...
            if (isCorrect) {
              ctx.userData.score++;
              feedback = `That's correct! The answer is ${ctx.userData.currentQuestion?.answer}. Great job!`;
            } else {
              feedback = `Sorry, that's not quite right. The correct answer is ${ctx.userData.currentQuestion?.answer}.`;
            }
            ...
            // Send score update to frontend (for live tracking)
            await this.sendScoreUpdateFromTool(ctx);
        ...
        end_game: llm.tool({
          description: 'End the trivia game and show final score summary',
          execute: async (_, { ctx }: llm.ToolOptions<TriviaState>) => {
            ...
            // Send final score update to frontend
            await this.sendScoreUpdateFromTool(ctx);

            // Update backend with final score
            await ctx.userData.updateBackendScore();

            const finalMessage = `Thanks for playing! Here's your final score summary: You got ${ctx.userData.score} out of ${ctx.userData.questionsAsked} questions correct. That's ${percentage.toFixed(1)} percent! Great job!`;
  ...
  private async sendScoreUpdateFromTool(ctx: any): Promise<void> {
    try {
      const scoreData = ctx.userData.getScoreData();
      ...
      // Send data to all participants via the room from userData
      if (ctx.userData.room) {
        await ctx.userData.room.localParticipant?.publishData(data, { reliable: true });
  ...
  async onEnter(): Promise<void> {
    // Send initial score (0/0) to frontend
    await this.sendInitialScore();
...
Just answer with what you think is correct. Ready to start? Let me ask you the first question!`,
  ...
  private async sendInitialScore(): Promise<void> {
    try {
      const scoreData = this.session.userData.getScoreData();
      const encoder = new TextEncoder();
      const data = encoder.encode(JSON.stringify({
        type: 'score_update',
        ...scoreData,
      ...
      if (this.session.userData.room) {
        await this.session.userData.room.localParticipant?.publishData(data, { reliable: true });
        console.log('Initial score sent:', scoreData);
      }
    } catch (error) {
      console.error('Error sending initial score:', error);
    }
  }
}
