...
export const ChatEntry = ({
  ...
  return (
    ...
    >
      ...
      >
        {name && <strong>{messageOrigin === 'local' ? '🎤 ' : '🤖 '}{name}</strong>}
      ...
      <span
        className={cn(
          'max-w-4/5 rounded-[20px] shadow-sm',
          messageOrigin === 'local' 
            ? 'bg-gradient-to-br from-primary/20 to-accent/10 border border-primary/30 ml-auto p-3 text-foreground' 
            : 'bg-gradient-to-br from-muted to-card border border-muted-foreground/20 mr-auto p-3'
        )}
      >
        {message}
      </span>
